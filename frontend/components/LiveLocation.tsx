...
const CATEGORIES = [
  ...
  },
  {
    ...
    label: "Mental Health",
    icon: Brain,
    query: '["healthcare:speciality"~"psychiatry|psychotherapy"]',
  },
  {
    id: "pharmacy",
    label: "Pharmacy",
...
export default function LiveLocation() {
  ...
  const lastFetchCoordsRef = React.useRef<Coordinates | null>(null);
  const abortControllerRef = React.useRef<AbortController | null>(null);

  // 📡 Overpass API Engine
  const fetchNearby = useCallback(
    async (lat: number, lng: number, radius: number = 2500) => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 429)
            throw new Error("Rate limit exceeded. Try again in 60s.");
          if (response.status === 504)
            throw new Error("Gateway timeout. Reducing search area...");
  ...
  useEffect(() => {
    if (userCoords) {
      const distanceMoved = lastFetchCoordsRef.current
        ? calculateDistance(lastFetchCoordsRef.current, userCoords)
        : Infinity;
    ...
    return (
      <Card className="m-6 p-12 text-center bg-red-50/50 border-red-100 rounded-3xl">
        ...
        <Button
          ...
          className="mt-8 rounded-full px-12 py-6 bg-red-600 hover:bg-red-700 shadow-xl shadow-red-600/20"
        >
          REBOOT SENSORS
        </Button>
      </Card>
    ...
    <div className="flex flex-col h-screen bg-white overflow-hidden border-0 pt-16 pb-16 lg:pt-0 lg:pb-0">
      ...
      {emergencyMode && (
        <div className="bg-red-600 text-white px-6 py-4 flex items-center justify-between animate-pulse shrink-0">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-white/20 hover:bg-white/40 rounded-full text-xs font-black uppercase transition-all"
          >
            DISMISS
          </button>
        </div>
      ...
      <div className="relative h-1/2 md:h-[55%] shrink-0 overflow-hidden bg-primary-100/30">
        {!geoLoading && userCoords ? (
          ...
          >
            ...
            {routeData.length > 0 && (
              <Polyline
                positions={routeData}
                color={emergencyMode ? "#ef4444" : "#22c55e"}
                weight={6}
                opacity={0.8}
              />
            )}
        ...
        <div className="absolute top-6 left-6 z-20 flex flex-col gap-3">
          ...
          <Button
            onClick={() =>
              userCoords && fetchNearby(userCoords.lat, userCoords.lng)
            }
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4 bg-primary-50/30">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-40 bg-white border border-primary-50 rounded-3xl animate-pulse p-8 space-y-4"
            >
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-3 rounded-2xl ${isSelected ? "bg-primary-500 text-white shadow-lg" : "bg-primary-50 text-primary-400 group-hover:bg-primary-100 transition-all duration-500"}`}
                      >
                        <activeCategory.icon className="w-6 h-6" />
                      </div>
                      <h4
                        className={`text-xl font-black tracking-tight leading-tight uppercase transition-all duration-500 ${isSelected ? "text-primary-900" : "text-primary-800"}`}
                      >
                        {f.name}
                      </h4>
                    </div>
                    <div className="flex flex-col gap-2 pl-12">
                      ...
                      <div className="flex items-center gap-2 text-primary-900 text-xs font-bold bg-primary-50/50 w-fit px-3 py-1 rounded-lg">
                        ...
                        {formatDistance(f.distance)} PROXIMITY
                      </div>
                    </div>
                  </div>
          ...
          <div className="py-24 text-center space-y-6">
            ...
            <div>
              <p className="font-black text-primary-900 uppercase tracking-widest text-sm">
                No Medical Nodes Detected
              </p>
              <p className="text-xs text-primary-400 mt-2 font-bold uppercase tracking-tight">
                Try expanding your search radius or sensor grid.
              </p>
            </div>
            <Button
              variant="outline"
              ...
              className="rounded-full px-12 py-5 font-black uppercase tracking-widest border-primary-100"
            >
              Scan Environment
            </Button>
          </div>
